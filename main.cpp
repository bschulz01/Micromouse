...
public:
    ourRobot(bool shouldPause = false) : pause(shouldPause) {
        shouldGoForward = false;
        visitedStart = false;
        ...
        int midpoint = MazeDefinitions::MAZE_LEN / 2;
        for (int i = 0; i < MazeDefinitions::MAZE_LEN; i++) {
            if (midpoint % 2 != 0) {
                midpoint++;
            }
            for (int j = 0; j < MazeDefinitions::MAZE_LEN; j++) {
                distances[i][j] = abs(midpoint - i) + abs(midpoint - j);
            }
        }
    }
...
int main(int argc, char * argv[]) {
    ...
    ourRobot ourRobot(pause);
    Maze maze(mazeName, &ourRobot);
    std::cout << maze.draw(5) << std::endl << std::endl;
